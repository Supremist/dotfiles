...
# Usage:
# Update-MsStoreApps @("Microsoft.WindowsTerminal_8wekyb3d8bbwe",
        ...
        foreach ($app in $appsToUpdate) {
            try {
                ...
                while (-not $completed) {
                    ...
                    $percent = $updateResult.GetCurrentStatus().PercentComplete
                    $completed = ($percent -eq 100)
                    Write-Progress -Activity $app -Status "Updating..." -PercentComplete $percent -Completed:$completed
                    if (-not $completed) {
